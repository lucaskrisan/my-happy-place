...
const TASKS: RoadmapTask[] = [
  ...
  task("PREVIEW-001", "FASE-4", "Prévia completa do funil usa o runtime real corretamente", {
    description: "GuidedPreview.tsx usava sempre o `scene` prop fixo (o da montagem do componente) para decidir vídeo/overlays, mesmo depois do FunnelRuntime avançar de cena via NEXT_SCENE/GO_TO_SCENE — então numa prévia multi-cena, o vídeo e as interações continuavam sendo os da PRIMEIRA cena para sempre. Corrigido: agora deriva a cena atual de `snapshot.sceneId` (mesmo padrão já correto em /dev/funnel-runtime-proof.tsx). HOTFIX (2026-08-22, achado no smoke real do usuário): mesmo com a cena certa, o vídeo da nova cena ficava parado em 0:00 — o efeito que chama play()/pause() só reage a snapshot.mediaState mudar de valor, e enterScene() deixa mediaState em 'playing' o tempo todo (vai idle→playing dentro da mesma chamada, antes do React observar o 'idle'), então o efeito nunca disparava de novo na troca de cena. Corrigido: o autoplay da nova cena agora é decidido no evento loadedmetadata do próprio elemento <video> (via onReady), lendo o snapshot ao vivo do runtime (runtime.snapshot(), não o estado React potencialmente desatualizado) — sem setTimeout nem delay arbitrário. O relógio visível (tempo exibido) também é resetado para 0 assim que a cena muda. RuntimeOverlays ganhou key={activeEvent.id} para garantir desmonte/montagem limpos entre eventos de cenas diferentes. Botão troca de 'TESTAR ESTA CENA' para 'INICIAR EXPERIÊNCIA' quando fullExperience=true (modo prévia completa).",
    status: "IN_PROGRESS", priority: "NOW",
    dependencies: ["E2E-001"],
    acceptanceCriteria: [
      "GuidedPreview deriva vídeo e overlays de snapshot.sceneId, não do prop `scene` fixo",
      "autoplay da nova cena é decidido no loadedmetadata do <video>, lendo o snapshot ao vivo do runtime — nunca com setTimeout/delay arbitrário",
      "relógio visível reseta para 00:00 na troca de cena",
      "teste automatizado renderiza o componente real e dirige uma transição real (VIDEO_END → NEXT_SCENE), validando o elemento <video> (src, play() chamado só após loadedmetadata) — não só snapshot.sceneId",
      "typecheck limpo",
      "validação real em browser autenticado, no funil oficial da Marina — PENDENTE, o dono faz esse smoke",
    ],
    internalNotes: "Corrigido o bug de estado (código) e, nesta rodada, o bug de autoplay real encontrado no smoke autenticado do usuário. NÃO revalidado ao vivo ainda — pendente novo smoke do dono no /studio de produção antes de marcar DONE.",
  }),
