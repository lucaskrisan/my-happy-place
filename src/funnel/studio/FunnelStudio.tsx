  ...
  return (
    <main className="h-screen min-h-[700px] bg-zinc-950 text-zinc-100 grid grid-rows-[52px_1fr_210px] overflow-hidden">
      ...
      <div className="fixed bottom-3 right-[350px] bg-zinc-900 border border-zinc-700 p-2 text-xs">
        <b>ARQUIVOS</b>
        {funnel.assets.map((a) => {
          const usages = findAssetUsages(funnel, a.id);
          return (
            <div key={a.id}>
              {a.id} —{" "}
              {a.source === "preview"
                ? urls[a.id]
                  ? "LOCAL — PREVIEW"
                  : "PRECISA SER REANEXADO"
                : "PERMANENTE"}{" "}
              {a.source === "preview" && <button onClick={() => attach(a.id)}>REANEXAR</button>}
              <button
                onClick={() => {
                  const detail = usages.map((item) => `• ${item.label}`).join("\n");
                  if (
                    confirm(
                      usages.length
                        ? `Este arquivo está sendo usado em ${usages.length} lugares:\n${detail}\n\nREMOVER MESMO ASSIM? As referências serão limpas.`
                        : "Excluir este arquivo não utilizado?",
                    )
                  ) {
                    const previousUrl = urls[a.id];
                    if (previousUrl) {
                    ...
                    change(removeAsset(funnel, a.id));
                  }
                }}
              >
                EXCLUIR
              ...
              <div className="text-zinc-500">
                ...
                {usages.length
                  ? usages.map((item) => (
                      <span className="block" key={item.path}>
                        {item.label}
                      </span>
                    ))
                  : "não utilizado"}
        ...
        <button
          onClick={() => {
            const url = prompt("URL permanente"),
              type = prompt("video, audio ou image", "video") as "video" | "audio" | "image";
            if (url && ["video", "audio", "image"].includes(type))
              change({
                ...funnel,
                assets: [
                  ...funnel.assets,
                  { id: uid(type), mediaType: type, source: "permanent", url },
                ],
              });
          }}
        >
